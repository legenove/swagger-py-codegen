...
class Router(Code):
    ...
    template = 'django/routers.tpl'
    dest_template = '%(package)s/%(module)s/urls.py'
    override = True


class View(Code):

    template = 'django/view.tpl'
    dest_template = '%(package)s/%(module)s/views/%(view)s.py'
    override = False


class Specification(Code):

    template = 'django/specification.tpl'
...
class Validator(Code):
    ...
    template = 'django/validators.tpl'
...
class Api(Code):
    ...
    template = 'django/api.tpl'
    dest_template = '%(package)s/%(module)s/views/__init__.py'


class Blueprint(Code):

    template = 'django/blueprint.tpl'
    dest_template = '%(package)s/%(module)s/__init__.py'


class App(Code):

    template = 'django/manage.tpl'
    dest_template = 'manage.py'


class Requirements(Code):

    template = 'django/requirements.tpl'
    dest_template = 'requirements.txt'


class Middleware(Code):

    template = 'django/middleware.tpl'
    dest_template = '%(package)s/middleware.py'


class Init(Code):

    template = 'django/empty.tpl'
    dest_template = '%(package)s/__init__.py'


class Settings(Code):

    template = 'django/settings.tpl'
    dest_template = '%(package)s/settings.py'


class Wsgi(Code):

    template = 'django/urls.tpl'
    dest_template = '%(package)s/urls.py'


class Urls(Code):

    template = 'django/wsgi.tpl'
    dest_template = '%(package)s/wsgi.py'
...
def _swagger_to_django_url(url, swagger_path_node):
...
class DjangoGenerator(CodeGenerator):
    ...
    def __init__(self, swagger):
        super(DjangoGenerator, self).__init__(swagger)
    ...
    def _process_data(self):
        ...
        for paths, data in self.swagger.search(['paths', '*']):
            ...
            url, params = _swagger_to_django_url(swagger_path, data)
    ...
    def _process(self):
        ...
        yield Wsgi(dict(package=self.swagger.package))

        yield Settings(dict(package=self.swagger.package))
        
        yield Urls(dict(package=self.swagger.package))
